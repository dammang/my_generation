...
TreeGraph _graph({
  ...
  people: {
    for (final entry in depths.entries)
      entry.key: _person(entry.key, entry.value),
  },
  unions: unions,
  edges: edges,
...
void main() {
  group('rows', () {
    ...
    test('people at the same depth share a row', () {
      ...
      final tops = [
        ...
        'b',
        'c',
      ].map((u) => layout.nodes[u]!.rect.top).toSet();
  ...
  group('couples', () {
    test('partners end up adjacent', () {
      ...
      expect(
        coupleGap < toOther,
        isTrue,
        reason: 'A couple sits closer together than to anybody else',
      );
    });
    ...
    test('a couple gets a bar between them', () {
      final layout = engine.layout(
        _graph(
          ...
          depths: {'a': 0, 'b': 0},
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['a', 'b'], childUlids: []),
          ],
        ),
      );
    ...
    test('a single parent gets no bar', () {
      ...
      final layout = engine.layout(
        _graph(
          ...
          depths: {'a': 0, 'child': 1},
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['a'], childUlids: ['child']),
          ],
  ...
  group('children', () {
    test('a parent is centred over their children', () {
      ...
      final children = [
        ...
        'b',
        'c',
      ].map((u) => layout.nodes[u]!.rect.center.dx).toList()..sort();
    ...
    test('children get a sibling bar and one drop each', () {
      final layout = engine.layout(
        _graph(
          ...
          depths: {'p': 0, 'a': 1, 'b': 1},
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['p'], childUlids: ['a', 'b']),
          ],
          edges: const [
            TreeEdge(parentUlid: 'p', childUlid: 'a'),
            TreeEdge(parentUlid: 'p', childUlid: 'b'),
    ...
    test('an only child gets no sibling bar', () {
      final layout = engine.layout(
        _graph(
          ...
          depths: {'p': 0, 'a': 1},
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['p'], childUlids: ['a']),
          ],
      ...
      expect(
        layout.unionShapes.single.siblingBar,
        isNull,
        reason: 'A zero-width bar is a smudge, not information',
      );
    });

    test('children are drawn in the birth order the server sent', () {
      // Deliberately named so that alphabetical order is the *reverse* of birth
      // order. The previous fixture used first/second/third, which sort the
      // right way by accident — so the ulid tie-break looked correct and the
      // seventh son was drawn second on a real family for months.
      final layout = engine.layout(
        _graph(
          focus: 'p',
          depths: {'p': 0, 'zeldest': 1, 'middle': 1, 'ayoungest': 1},
          unions: const [
            TreeUnion(
              ulid: 'u1',
              partnerUlids: ['p'],
              childUlids: ['zeldest', 'middle', 'ayoungest'],
            ),
          ],
          edges: const [
            TreeEdge(parentUlid: 'p', childUlid: 'zeldest'),
            TreeEdge(parentUlid: 'p', childUlid: 'middle'),
            TreeEdge(parentUlid: 'p', childUlid: 'ayoungest'),
          ],
        ),
      );

      // Positions, not the drop list: where they actually sit on the canvas is
      // what somebody looks at and calls wrong.
      final byX = layout.nodes.values.where((n) => n.ulid != 'p').toList()
        ..sort((a, b) => a.rect.left.compareTo(b.rect.left));

      expect(byX.map((n) => n.ulid).toList(), [
        'zeldest',
        'middle',
        'ayoungest',
      ]);
    ...
    test('the youngest of a large family is drawn last', () {
      // The reported case: seven children, the seventh entered early enough
      // that his record sorted second.
      const children = [
        'sinte',
        'pante',
        'kawngte',
      ...
      final layout = engine.layout(
        _graph(
          focus: 'jasuan',
          depths: {'jasuan': 0, for (final child in children) child: 1},
          unions: const [
            TreeUnion(
              ulid: 'u1',
              partnerUlids: ['jasuan'],
      ...
      final byX = layout.nodes.values.where((n) => n.ulid != 'jasuan').toList()
        ..sort((a, b) => a.rect.left.compareTo(b.rect.left));
    ...
    test('an adoptive child is marked for a dashed drop', () {
      final layout = engine.layout(
        _graph(
          ...
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['p'], childUlids: ['adopted']),
          ],
          edges: const [
            TreeEdge(
              parentUlid: 'p',
    ...
    test('an edge already drawn by a union is not drawn twice', () {
      final layout = engine.layout(
        _graph(
          ...
          depths: {'p': 0, 'c': 1},
          unions: const [
            TreeUnion(ulid: 'u1', partnerUlids: ['p'], childUlids: ['c']),
          ],
          edges: const [TreeEdge(parentUlid: 'p', childUlid: 'c')],
        ),
      );
    ...
    test('viewport culling returns only what overlaps', () {
      ...
      final layout = engine.layout(
        _graph(
          ...
          edges: [
            for (var i = 0; i < 20; i++)
              TreeEdge(parentUlid: 'p', childUlid: 'c$i'),
          ],
        ),
      );
  ...
  group('robustness', () {
    ...
    test('a lone person is a valid tree', () {
      final layout = engine.layout(
        _graph(focus: 'alone', depths: {'alone': 0}),
      );
    ...
    test('the same graph always lays out identically', () {
      ...
      for (final ulid in first.nodes.keys) {
        expect(
          first.nodes[ulid]!.rect,
          second.nodes[ulid]!.rect,
